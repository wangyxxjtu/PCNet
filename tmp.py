...
class CenterCrop(object):
	...
	def __call__(self, inputs, target):
		...
		bd_local = (bd_local > 0.).astype(np.float32)

		return [inputs[0],target, inputs[0], bd_local[:,:,None], np.ones((target.shape[0], target.shape[1], 2)), np.ones((2,2,1))*6], target
...
class RandomCropBoundary(object):
	def __init__(self, height=512, width=512):
		self.height = height//2 
		self.width = width//2
		self.ones = np.ones((self.height*2, self.width*2, 1))
		self.zeros = np.zeros((self.height*2, self.width*2, 1))
	
	def get_binary_label(self, target, bd):
		bd_ = ((target+1)*bd).astype(np.int64)
		ele, count = np.unique(bd_, return_counts=True)
		bd_index = np.argsort(count)
		fg_index = ele[bd_index[-1]] if ele[bd_index[-1]]!=0 else ele[bd_index[-2]]
		fg_index = fg_index-1
		fg_label = np.where(target==fg_index, self.ones, self.zeros)
		bk_label = (1-fg_label)

		return np.concatenate([fg_label, bk_label], axis=-1), fg_index
	
	def get_classifier_indicator(self, fg_label, face):
		#foreground labels for face  is 7,4,0,16
		#foreground labels for human is 0, 11
		#the there will be five different number of indicator the class
		if fg_label == 0:
			return 0
		if face:
			if fg_label==7:
	...
	def pick_up_index(self, bd_index, h, w):
		rand_idx = random.randint(0,len(bd_index[0])-1)
		y,x = bd_index[0][rand_idx], bd_index[1][rand_idx]		 
			
		y_start,y_end=y-self.height,y+self.height
		x_start,x_end=x-self.width,x+self.width
		if y_start<0:
			y_end = y_end - y_start 
			y_start = 0
		elif y_end>h:
	...
	def random_crop(self, im,gt, h=768, w=768):
		x1 = random.randint(0, w - 512)
		y1 = random.randint(0, h - 512)

		im = im[y1: y1 + 512,x1: x1 + 512]
		gt = gt[y1: y1 + 512,x1: x1 + 512]
		return im, gt[:,:,None]
		
	def __call__(self, inputs, target):
		bd = inputs[1]
		face = inputs[-1]
		im_resize= cv2.resize(inputs[0], (768, 768))
		target_resize= cv2.resize(target, (768, 768)).round()
		im_full, gt_full = self.random_crop(im_resize, target_resize)

		success = False
		h,w,c = inputs[0].shape
		bd_index = np.where(bd>0)	
		
		if len(bd_index[0])<2:
			for i in range(len(inputs)):
				inputs[i] = inputs[i][:512,:512]
				target = target[:512,:512]
				bd_local = np.ones((512,512,1))
				bd_label = np.ones((512,512,2))

			return [inputs[0], target, inputs[0], bd_local, bd_label], target

		y_start, y_end, x_start, x_end = self.pick_up_index(bd_index, h, w)    
		for i in range(len(inputs)-1):
			inputs[i] = inputs[i][y_start:y_end, x_start:x_end]

		target = target[y_start:y_end, x_start:x_end]
		bd = inputs[1]
		binary_label, fg_label = self.get_binary_label(target, bd) 
		class_indicator = self.get_classifier_indicator(fg_label, face)

		bd_local = cv2.dilate(bd, kernel=np.ones((16,16)), iterations=1)
		bd_local = (bd_local > 0.).astype(np.float32)
		
		return [im_full, gt_full, inputs[0], bd_local[:,:,None], binary_label, np.ones((2,2,1))*class_indicator], target
