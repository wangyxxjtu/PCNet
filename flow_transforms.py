...
class CenterCrop(object):
	...
	def __call__(self, inputs, target):
		...
		return [inputs[0],target, bd_local[:,:,None], inputs[0], bd_local[:,:,None], np.ones((target.shape[0], target.shape[1], 2))], target
...
class RandomCropBoundary(object):
	...
	def get_binary_label(self, target, bd):
		...
		return np.concatenate([fg_label, bk_label], axis=-1)
	...
	def random_crop(self, im, gt, bd,h=768, w=768):
		x1 = random.randint(0, w - 512)
		y1 = random.randint(0, h - 512)

		im = im[y1: y1 + 512,x1: x1 + 512]
		gt = gt[y1: y1 + 512,x1: x1 + 512]
		bd = bd[y1: y1 + 512,x1: x1 + 512]
		return im, gt[:,:,None], bd
		
	def __call__(self, inputs, target):
		bd = inputs[1]
		im_resize= cv2.resize(inputs[0], (768, 768))
		target_resize= cv2.resize(target, (768, 768)).round()
		bd_resize= cv2.resize(bd.astype(np.float32), (768, 768)).round()
		#pdb.set_trace()	
		im_full, gt_full, bd_full = self.random_crop(im_resize, target_resize, bd_resize)
		...
		bd = inputs[1]
		binary_label = self.get_binary_label(target, bd) 

		bd_local = cv2.dilate(bd, kernel=np.ones((16,16)), iterations=1)
		bd_local = (bd_local > 0.).astype(np.float32)

		full_bd_local = cv2.dilate(bd_full, kernel=np.ones((16,16)), iterations=1)
		full_bd_local = (full_bd_local > 0.).astype(np.float32)

		return [im_full, gt_full, full_bd_local[:,:,None], inputs[0], bd_local[:,:,None], binary_label], target
